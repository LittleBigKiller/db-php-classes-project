    ...
    <div id="loggedin_box">
      ...
      if ($is_loggedin) {
        ...
        <div id="loggedin_text">
          Zalogowany jako: <b>
            <?php
              echo (str_replace('\\"', '"', $_COOKIE['username']));
              ?>
          </b>
        </div>
    ...
    <div id="scroll_content_box">
      ...
      if ($is_loggedin) {
        ...
        <?php
                      ...
                      if ($rec1['aid'] == $_POST['aid_q' . $q_num]) {
                        echo '
                                    <tr class="result_good">
                                        <td class="result_radans"><input type="radio" checked onclick="event.preventDefault()" name="aid_q' . $q_num . 'yeet' . $a_num . '"><label for="aid_q' . $q_num . 'yeet' . $a_num . '">  ' . chr($a_num) . '. ' . $answer . '</label></td>
                                    </tr>';
                        ...
                        echo '
                                    <tr class="result_good">
                                        <td class="result_radans"><input type="radio" onclick="event.preventDefault()" name="aid_q' . $q_num . 'yeet' . $a_num . '"><label for="aid_q' . $q_num . 'yeet' . $a_num . '">  ' . chr($a_num) . '. ' . $answer . '</label></td>
                                    </tr>';
                      }
          ...
          ?>
        <div class="result_score_box" style="order: 1;">
          Test zakończony. <span class="result_score_yourscore">Twój wynik to: <?php echo (($num_correct / 10) * 100 . '% (' . $num_correct . '/10'); ?>)</span>
        </div>

        <div class="result_score_box" style="order: 2;">
          Test zakończony. <span class="result_score_yourscore">Twój wynik to: <?php echo (($num_correct / 10) * 100 . '% (' . $num_correct . '/10'); ?>)</span>
        </div>
        <?php
          $uid = 0;
